...
/* harmony default export */ __webpack_exports__["default"] = ({
    ...
    data() {
        return {
            ...
            sendUrl: this.api + 'inVestGoldOrder/sendCargo',
            currentPage: 0,
            pagesize: 10,
            pageNum: 1,
            totalNum: 0,
    ...
    methods: {
        handleSelectionChange(val) {
            ...
            this.selectRows = [];
            for (var i = 0; i < this.multipleSelection.length; i++) {
                // alert(userIds.contains(this.multipleSelection[i].userId))
                if (userIds.indexOf(this.multipleSelection[i].userId) === 0) {} else {
                    this.selectRows.push(this.multipleSelection[i]);
                    this.flag = 1;
                    this.$message({
                        message: '订单必须属于同一一个客户',
                        type: 'warning'
                    });
                }
                if (this.flag != 1) {
                    if (this.multipleSelection[i].status != "未发货") {
                        this.selectRows.push(this.multipleSelection[i]);
                        this.flag = 1;
                        this.$message({
                            message: '订单必须是未发货状态',
                        ...
                        if (phoneList.length > 0) {
                            if (phoneList.indexOf(this.multipleSelection[i].contactPhone) === 0 && addressList.indexOf(this.multipleSelection[i].deliveryAddress) === 0 && nameList.indexOf(this.multipleSelection[i].contactName) === 0) {} else {
                                this.selectRows.push(this.multipleSelection[i]);
                                this.flag = 1;
                                this.$message({
                                    message: '订单的收货信息必须相同',
                        ...
                        phoneList.push(this.multipleSelection[i].contactPhone);
                        addressList.push(this.multipleSelection[i].deliveryAddress);
                        nameList.push(this.multipleSelection[i].contactName);
                        this.orderIds.push(this.multipleSelection[i].id);
                        this.userId = this.multipleSelection[i].orderNo;
                        this.name = this.multipleSelection[i].contactName;
                        this.phone = this.multipleSelection[i].contactPhone;
                        this.address = this.multipleSelection[i].deliveryAddress;
                    }
                }
            }
            if (this.flag === 1) {
        ...
        sendCargo(clickCount) {
            ...
            if (clickCount == 1) {
                params.append("orderId", "JQJG" + this.dateFormat1(new Date()));
            } else {
                params.append("orderId", this.newformAdd.id);
            }

            params.append("orderIds", this.orderIds);
            params.append("clickCount", clickCount);
            params.append("mailNo", this.newformAdd.logisticsNo);
            axios.post(this.sendUrl, params).then(function (response) {
                _this.isClick = false;
                if (response.data.code == 1001) {
                    _this.isClick = false;
                    if (clickCount == 1) {
                        ...
                        _this.newformAdd.logisticsNo = response.data.data.split(",")[0];
                        _this.newformAdd.id = response.data.data.split(",")[1];
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v("导出")])], 1), _vm._v(" "), (_vm.userName == '田慧') ? _c('el-form-item', {
    attrs: {
      "fixed": "right",
      "width": "200"
    }
  ...
  }), _vm._v(" "), (_vm.userName == '田慧') ? _c('el-table-column', {
    attrs: {
      "label": "操作",
      "fixed": "right",
      "width": "200"
    ...
    scopedSlots: _vm._u([{
      key: "default",
      fn: function(scope) {
        return [(scope.row.status == '已发货') ? [_c('el-button', {
          attrs: {
            "size": "small",
            "type": "danger"
          },
  ...
  }) : _vm._e()], 1)], _vm._v(" "), _c('br'), _c('br'), _vm._v(" "), _c('el-pagination', {
    attrs: {
...
var update = __webpack_require__(198)("450a4e7b", content, true);
