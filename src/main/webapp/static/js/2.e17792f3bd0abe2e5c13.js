...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_highcharts__ = __webpack_require__(664);
...
/* harmony default export */ __webpack_exports__["default"] = ({
    data() {
        return {
            areaUrl: "inOutGold/selectByRechargeChannel",
            agentUrl: "inOutGold/selectByAgent",
            agentNetUrl: "inOutGold/selectByAgentNet",
            form: {
                radio: '',
                startTime: "",
                endTime: ""
    ...
    methods: {
        ...
        onRadioButtonsChange1() {
            if (this.form1.radio != 0) {
        ...
        onSubmit(form) {
            ...
            if (this.form.endTime != '') {
                endTime = this.dateFormat(this.form.endTime);
            }
            if (this.form.startTime != '') {
                startTime = this.dateFormat(this.form.startTime);
            }
            var params = new URLSearchParams();
            let _this = this;
            params.append('type', this.form.radio);
            params.append('startTime', startTime);
            params.append('endTime', endTime);
            axios.post(this.areaUrl, params).then(function (response) {
                var preList = response.data.data;
                var uionPay = [],
                    aliPay = [],
                    weChatPay = [];
                for (var i = 0; i < preList.length; i++) {
                    if (preList[i].rechargeChannel == '01') {
                        uionPay.push([preList[i].rechargeTime * 1000, preList[i].rmbAmt]);
                    } else if (preList[i].rechargeChannel == '02') {
                        weChatPay.push([preList[i].rechargeTime * 1000, preList[i].rmbAmt]);
                    } else if (preList[i].rechargeChannel == '03') {
                        aliPay.push([preList[i].rechargeTime * 1000, preList[i].rmbAmt]);
                    }
                }
                console.log(uionPay);
        ...
        onSubmit1(form1) {
            ...
            if (this.form1.endTime != '') {
                endTime = this.dateFormat(this.form1.endTime);
            }
            if (this.form1.startTime != '') {
                startTime = this.dateFormat(this.form1.startTime);
            }
            var params = new URLSearchParams();
            let _this = this;
            params.append('type', this.form1.radio);
            params.append('startTime', startTime);
            params.append('endTime', endTime);
            axios.post(this.agentUrl, params).then(function (response) {
                var crjArr = response.data.data;
                var rjArr = [],
                    cjArr = [],
                    nameArr = [];
                for (var i = 0; i < crjArr.length; i++) {
                    nameArr.push(crjArr[i].agentName);
                    cjArr.push(crjArr[i].cjSum);
                    rjArr.push(crjArr[i].rjSum);
                }
                __WEBPACK_IMPORTED_MODULE_0_highcharts___default.a.chart('container1', {
                    chart: {
                        type: 'bar',
                        width: 600,
                        height: 400
            ...
            axios.post(this.agentNetUrl, params).then(function (response) {
                var jingArr = response.data.data;
                var jingXDataArr = [],
                    jingYDataArr = [];
                for (var i = 0; i < jingArr.length; i++) {
                    jingXDataArr.push(jingArr[i].agentName);
                    jingYDataArr.push(jingArr[i].jrjSum);
                }
                // console.log(jingDataArr);
                __WEBPACK_IMPORTED_MODULE_0_highcharts___default.a.chart('container2', {
                    title: {
                        text: '运营商净入金分析 ' //指定图表标题
                    },
                    chart: {
                        type: 'column',
                        width: 600,
...
// module
exports.push([module.i, ".container-fluid[data-v-9f9908f0]{margin:30px;padding:0;background:#eee}.header[data-v-9f9908f0]{color:#000;height:60px;line-height:60px;background:#f9f9f9;text-indent:20px;font-size:20px;border:1px solid #cdcdcd}.chartContent[data-v-9f9908f0]{background:#fff;border:1px solid #cdcdcd;border-top:none}.container[data-v-9f9908f0]{display:inline-block}", ""]);
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_c('el-form-item', [_c('el-radio-group', {
    attrs: {
      "size": "small",
      "prop": "form.radio"
    },
  ...
  }, [_vm._v("近7天")]), _vm._v(" "), _c('el-radio-button', {
    attrs: {
      "label": "4"
    }
  }, [_vm._v("本月")])], 1)], 1), _vm._v(" "), _c('el-form-item', {
    attrs: {
      "label": "自定义时间",
      "prop": "startTime"
    }
    ...
    on: {
      "change": function($event) {
        _vm.onRadioButtonsChange1()
      }
    },
    model: {
      value: (_vm.form1.radio),
      callback: function($$v) {
        _vm.$set(_vm.form1, "radio", $$v)
      },
    ...
    model: {
      value: (_vm.form1.startTime),
      callback: function($$v) {
        _vm.$set(_vm.form1, "startTime", $$v)
      },
      expression: "form1.startTime"
    }
  })], 1)], 1), _vm._v(" "), _c('el-form-item', {
    attrs: {
      "prop": "endTime"
    ...
    model: {
      value: (_vm.form1.endTime),
      callback: function($$v) {
        _vm.$set(_vm.form1, "endTime", $$v)
      },
      expression: "form1.endTime"
    }
  })], 1)], 1), _vm._v(" "), _c('el-form-item', [_c('el-button', {
    attrs: {
      "size": "small",
...
var update = __webpack_require__(198)("67d503e6", content, true);
