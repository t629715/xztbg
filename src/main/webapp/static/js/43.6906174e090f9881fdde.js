    ...
    handelData(list) {
      if (list.length > 0) {
        for (var i = 0; i < list.length; i++) {
          list[i].cbf = this.amountHandle1(list[i].cbf);
          list[i].cj = this.amountHandle1(list[i].cj);
          list[i].rj = this.amountHandle1(list[i].rj);
          list[i].sjAmount = this.amountHandle1(list[i].sjAmount);
          list[i].totalIncome = this.amountHandle1(list[i].totalIncome);
          list[i].finance = this.amountHandle1(list[i].finance);
          list[i].rmb = this.amountHandle1(list[i].rmb);
        }
      }
    },
    //根据代理商取经纪人列表
...
var update = __webpack_require__(198)("6d6c1362", content, true);
